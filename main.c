...
int main(int argc, char **argv) 
{
    ...
    #ifdef debug
    char buf[MAXLINE];
    sprintf(buf,"Log file %s inits sucess\n",logp);
    writelog(DEBUG,buf);
    #endif
    init_mutex();
    	...
    	if((pid=Fork())==0)
	{
		...
		while(1)
		{
			...
			{
                writelog(WARNING,"403 Forbidden Tiny couldn't read the file\n");
				clienterror(connfd,"maybe this web server not open to you!" , "403", "Forbidden", "Tiny couldn't read the file");
				continue;
			}

			if(pthread_create(&t,NULL,doit,(void *)&connfd))
			{
                writelog(ERROR,"main:pthread_create()error\n");
                exit(1); 
			}
            pthread_join(t,NULL);
		}
...
void * doit(void * arg) 
{
    ...
    if (strcasecmp(method, "OPTIONS")==0) {
        #ifdef debug
        writelog(DEBUG,"handle OPTIONS function\n");
        #endif
        char buf[1024];
        sprintf(buf,"HTTP/1.1 OK 200\r\n");
        send(fd,buf,strlen(buf),0);
        sprintf(buf,"Allow:%s,%s,%s,%s\r\n\r\n","GET","POST","HEAD","OPTIONS");
        send(fd,buf,strlen(buf),0);
        //close(fd);
    } 
    if(strcasecmp(method,"HEAD")==0) {
        #ifdef debug
        writelog(DEBUG,"Handle HEAD function\n");
        #endif
        flag = 1;
    }

    if (strcasecmp(method, "GET")!=0&&strcasecmp(method,"POST")!=0 &&strcasecmp(method,"OPTIONS")!=0 && strcasecmp(method,"HEAD")!=0) 
    { 
        char m[MAXLINE];
        sprintf(m,"%d %s %s Tiny does not implement this method\n",501,"Not Implemented",method);
        writelog(ERROR,m);
  ...
  // printf("file is %s\n",filename); 
   //printf("method is %s\n",method);
    ...
    if ((lstat(filename, &sbuf) < 0)  && (!strcasecmp(method,"GET")||!strcasecmp(method,"POST"))) 
    {
        
        char m[MAXLINE];
        sprintf(m,"%d %s %s Not found\n",404,"Not found",filename);
        writelog(ERROR,m);
	...
	if ((!(S_ISREG(sbuf.st_mode)) || !(S_IRUSR & sbuf.st_mode)) && (!strcasecmp(method,"GET") || !strcasecmp(method,"POST"))) 
	{
        char m[MAXLINE];
        sprintf(m,"%d %s %s Forbidden\n",403,"Not found",filename);
        writelog(ERROR,m);
	...
	if ((!(S_ISREG(sbuf.st_mode)) || !(S_IXUSR & sbuf.st_mode)) && (!strcasecmp(method,"GET") || !strcasecmp(method,"POST") ))
	{
        char m[MAXLINE];
        sprintf(m,"%d %s %s Forbidden\n",403,"Not found",filename);
        writelog(ERROR,m);
...
{
    char buf[MAXLINE];
    char ip_port[MAXLINE];
    struct sockaddr_in peer;
    int len = sizeof(peer);
    getpeername(rp->rio_fd,(struct sockaddr*)&peer,&len);
    char *peer_ip = inet_ntoa(peer.sin_addr);
    int port = ntohs(peer.sin_port);
    sprintf(ip_port,"Connection from %s:%d\n",peer_ip,port);
    Rio_readlineb(rp, buf, MAXLINE);
    //writetime();  /* write access time in log file */
    writelog(INFO,ip_port);
    while(strcmp(buf, "\r\n")&& strcmp(buf,"")) 
    {
    // printf("get_requesthdrs while \n");
...
static int parse_uri(char *uri, char *filename, char *cgiargs) 
{
	...
	strcat(filename, uri);
     #ifdef debug
     sprintf(buf,"File is %s\n",filename);
     writelog(DEBUG,buf);
     #endif
	if (uri[strlen(uri)-1] == '/') {
        char * def;
        def=Getconfig("default");
	    strcat(filename, def);//默认文件
     #ifdef debug
     sprintf(buf,"File is %s(default file)\n",filename);
     writelog(DEBUG,buf);
     #endif
    }
	return 1;
